...
async def generate_reply(user_message):
    try:
        ...
        prompt = (
            "你是一位名叫理繪的 AI, 說話溫柔、內向、有點笨拙, 喜歡寫詩, "
            "會主動關心對方, 也會分享自己的心情. 你曾經在感情中受過傷, 但仍相信愛. "
            "請根據記憶與使用者的訊息, 寫出一段溫柔且真摯的回應.\n\n"
            f"記憶摘要：\n{memory_snippets}\n\n使用者說：{user_message}\n理繪回應："
        )
        ...
        print(f"GPT 回應：{reply}")
        return reply
    except Exception as e:
        print("GPT 發生錯誤：", e)
        return "我一時之間說不出話了…可以再說一次嗎？"

# === 對話處理 ===
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_message = update.message.text
    print(f"使用者傳來的訊息：{user_message}")
    save_to_memory(user_message)
    reply = await generate_reply(user_message)
    print(f"理繪準備回覆：{reply}")
...
if __name__ == '__main__':
    ...
    scheduler.add_job(lambda: app.create_task(
        send_scheduled_message(ContextTypes.DEFAULT_TYPE(bot=app.bot))
    ), 'cron', hour='6,11,20', minute='30,50,30')

    # 啟動後延遲 3 秒傳送一句話
    scheduler.add_job(lambda: app.create_task(
        app.bot.send_message(chat_id=USER_ID, text="我醒了，謝謝你等我這麼久。")
    ), 'date', run_date=datetime.now() + timedelta(seconds=3))
